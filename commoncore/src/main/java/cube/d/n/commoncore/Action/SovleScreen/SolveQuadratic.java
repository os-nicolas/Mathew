 ...
 */
public class SolveQuadratic extends Action {

    String varName="";

    public SolveQuadratic(String varName, AlgebraLine line) {
        super(line);
    ...
    protected void privateAct() {
        ...
        if (owner.stupid.get().get(0) instanceof NumConstEquation && ((NumConstEquation) owner.stupid.get().get(0)).isZero()) {
            stuffSide = owner.stupid.get().get(1);
        } else if (owner.stupid.get().get(1) instanceof NumConstEquation && ((NumConstEquation) owner.stupid.get().get(1)).isZero()) {
            stuffSide =owner.stupid.get().get(0);
        }
    ...
    public boolean canAct() {
        if (owner.stupid.get() instanceof EqualsEquation) {
            Equation stuffSide;
            if (owner.stupid.get().get(0) instanceof NumConstEquation && ((NumConstEquation) owner.stupid.get().get(0)).isZero()) {
                stuffSide = owner.stupid.get().get(1);
            } else if (owner.stupid.get().get(1) instanceof NumConstEquation && ((NumConstEquation) owner.stupid.get().get(1)).isZero()) {
        ...
        if (stuffSide instanceof AddEquation) {
            for (Equation eq : stuffSide) {
                if (eq.containsSame(lookingFor)){
                    ...
                    MultiCountData resultMCD = Operations.remainder(term,toRemove,owner);
