...
public class Pop extends Animation {
    float targetRadius = 32* BaseApp.getAlgebrator().getDpi();
    long runTime = 180;
    long startedAt;
    int startAlpha = 0x90;
    public MyPoint myPoint;

    public Pop(MyPoint myPoint,BaseView owner) {
        super(owner);
    ...
    public void draw(Canvas canvas) {
        ...
        if ((now - startedAt) < runTime) {
            ...
            float rad = 25*BaseApp.getAlgebrator().getDpi()+((targetRadius)*((now - startedAt)/(float)runTime));
