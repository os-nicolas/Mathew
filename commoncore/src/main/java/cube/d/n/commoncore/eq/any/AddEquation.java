...
public class AddEquation extends FlexOperation implements BinaryOperator {
    ...
    public ArrayList<SelectedRow> getSelectedRow() {
        ...
        if (this.size() == 2) {
            ...
            if (!Operations.add_canAddNumber(left, right, owner) && Operations.add_canZero(left,right,owner)){
                buttons.add(new SeletedRowEquationButton(Operations.add_Zero(new MultiCountData(left),new MultiCountData(right), owner), new Action(owner) {
                    @Override
                    protected void privateAct() {
                        MyPoint p = that.getNoneNullLastPoint(that.getX(),that.getY());
                        operateRemove(eqs);
                        Equation result = Operations.add_Zero(left, right, owner);
                        handleResult(0, result);
